...
static bool AddToServerList( const char *name, const char *label, const char* version, int clients, int bots,
...
void CG_Rocket_BuildServerList()
{
	...
	for ( i = 0; i < numServers; ++i )
	{
		...
		if ( ping > 0 )
		{
			...
			const std::string version = Info_ValueForKey( info, "version" );
			rocketInfo.data.haveServerInfo[ netSrc ][ i ] =
				AddToServerList( Info_ValueForKey( info, "hostname" ), Info_ValueForKey( info, "label" ),
					version.c_str(), clients, bots, ping, maxClients, mapname, addr, netSrc);
		}
	}
	...
	for ( i = oldServerCount; i < rocketInfo.data.serverCount[ netSrc ]; ++i )
	{
		...
		std::string version;
		if ( !Q_stricmp( rocketInfo.data.servers[netSrc][i].version, ENGINE_VERSION ) ) {
...
{
	int netSrc = CG_StringToNetSource( table );
	if ( Q_stricmp( rocketInfo.data.servers[netSrc]->version, ENGINE_VERSION ) ) {
